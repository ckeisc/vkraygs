...
REM (run the CMake build first; see README section "Building on Windows").
REM Requires Python 3.8+ with numpy and Pillow:  pip install numpy Pillow
...
if not exist "%VIEWER%" (
    ...
    echo Build it first (see README "Building on Windows").
    exit /b 1
)
